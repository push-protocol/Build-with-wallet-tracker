...
route.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    const logger: any = Container.get('logger');
    logger.debug('Calling /showrunners/trackDaoProposals ticker endpoint with body: %o', req.body);
    try {
      const wt = Container.get(WallettrackerChannel);
     //  const response = await wt.checkDaoProposals();
      const response = triggerDaoNotifications("proposals/0x56469f7675c8c3193a74a3c5fe502e63f992ce6d686e217f4c949fc4335d45b3");
      return res.status(201).json({ success: true, data: response });
    } catch (e) {
      logger.error('🔥 error: %o', e);
      return next(e);
...
route.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    const logger: any = Container.get('logger');
    logger.debug('Calling /showrunners/trackTransfers ticker endpoint with body: %o', req.body);
    try {
      const wt = Container.get(WallettrackerChannel);
     //  const response = await wt.checkDaoProposals();
