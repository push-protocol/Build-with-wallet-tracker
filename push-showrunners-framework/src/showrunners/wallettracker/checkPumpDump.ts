        ...
        if (userData.itemcount > 0) {
            ...
            for (let j = 0; j < subscribers.length; j++) {
                if (subscribers[j].settings !== null) {
                    const settings = JSON.parse(subscribers[j].settings)[1]; //Change index
                    if (settings.user === true) {
                        console.log("sending Notification to ", subscribers[j].subscriber,"Payload length ", payload.length);
                        await userAlice.channel.send([`${subscribers[j].subscriber}`], {
                            notification: { title: 'Is this a PUMP or DUMP, Let’s check out 👇 ', body: `Here's a list of tokens that went through pump/dumps` },
                            payload: {
...
function analyzePriceChange(cryptos: Coin[]) {
    ...
    cryptos.forEach((coin, index) => {
        ...
        if (index <21 && change >= 6.5 || index > 21 && change >= 10) {
            message = `<span color='#FF9100'>${(coin.name)}</span> (${(coin.symbol).toUpperCase()}) soars <span color='#6256CA'>${change.toFixed(2)}%</span>\n`;
            ...
            message = `<span color='#384B70'>${coin.name}</span> (${(coin.symbol).toUpperCase()}) takes a <span color='#507687'>${change.toFixed(2)}% dip </span>\n`;
